...
```bash
# Create .env with the public API URL and optional telemetry configuration
...
Authentication uses the API's custom email OTP endpoints. In local development,
set `RESEND_API_KEY` and `RESEND_FROM_EMAIL` to deliver codes, or use the API's
documented development-code behavior.
...
AUTH_JWT_SECRET=<64-hex-character-secret>
AUTH_OTP_PEPPER=<different-64-hex-character-secret>
...
```bash
# Auto-deploys development from `develop` and production from `prod`
...
- [ ] Auth flow: request and verify a custom email OTP
